...
public class Info extends Command {
  ...
  protected void execute(CommandEvent event) {
    ...
    boolean hasPermission =
        event.getMember().getRoles().stream().map(Role::getName).anyMatch(e -> e.equals(addRole));
    ...
    if (args[0].equals("topics")) {
      ...
      topics.forEach(
          topic ->
              sb.append(String.format("%4s", listNumber[0]++ + ". ")).append(topic).append("\n"));
    ...
    switch (args[0]) {
      case "add":
        if (args.length != 3) {
          event
              .getChannel()
              .sendMessage("Sorry, invalid number of arguments for add/update")
              .queue();
          return;
        }
