...
public class CodeBlockListener extends ListenerAdapter {
    ...
    private void handleFormattingCodeBlock(Message message) {
        ...
        newMessage.queue(sentMessage -> {
            ...
            sentMessage.addReaction(THUMBS_UP).queue(then -> sentMessage.addReaction(THUMBS_DOWN)
                    .queue(last -> sentMessage.addReaction(DELETE).queue()));
            sentMessage.clearReactions().queueAfter(5, TimeUnit.MINUTES,
                    (delete) -> codeBlocks.remove(codeBlock.getMessageId()),
                    (error) -> codeBlocks.remove(codeBlock.getMessageId()));
        });
    }
